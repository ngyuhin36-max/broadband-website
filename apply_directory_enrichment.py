"""Regenerate directory pages that got Wikipedia matches with richer content."""
import json, html
...
def render(slug, d):
    ...
    price = html.escape(d["price"]); orig = html.escape(d["orig"])
    extract = html.escape(d["extract"])
    img = d.get("image") or ""
    wiki_title = html.escape(d["wiki_title"])
    wiki_url = d["wiki_url"]
    hero_bg = f"background-image:linear-gradient(rgba(26,26,46,0.55),rgba(15,52,96,0.75)),url('{img}');background-size:cover;background-position:center;" if img else "background:linear-gradient(135deg,#1a1a2e,#16213e 50%,#0f3460);"
    schema = {
        "@context":"https://schema.org","@type":"Hotel",
        "name":d["name"],
        "description":d["extract"][:200],
...
<meta name="description" content="{name}香港住宿格價：優惠價{price}起（原價{orig}）。{extract[:80]} Trip.com／Klook／Agoda 即時比較。">
<meta name="keywords" content="{name}, 香港酒店, 格價, 2026, Trip.com, Klook">
<meta property="og:title" content="{name}｜格價比較．Broadband HK">
<meta property="og:description" content="{extract[:100]}">
...
<h2>關於 {name}</h2>
<p>{extract}</p>
<div class="wiki-note">📖 資料來源：<a href="{wiki_url}" target="_blank" rel="noopener noreferrer">維基百科 — {wiki_title}</a>（CC BY-SA 授權）</div>
</div>

<div class="card">
<h2>價格與預訂</h2>
<p><strong>目前優惠價：</strong>{price}／晚起（原價 {orig}，已折扣）</p>
<p><strong>建議：</strong>同一間酒店喺 Trip.com、Klook、Agoda 三個平台價錢可能差 15-30%，建議逐個比較。旺季（聖誕、農曆新年、煙花期）可加價 30-80%，建議提早 4-8 週預訂。</p>
</div>

<div class="card">
<h2>預訂香港酒店 3 大貼士</h2>
<ul>
<li><strong>提早預訂</strong>：旺季房價可升 50-100%，提早 4-8 週預訂鎖定優惠。</li>
<li><strong>揀免費取消</strong>：優先選 Free Cancellation 房型，行程有變唔會蝕錢。</li>
<li><strong>三大平台格價</strong>：Trip.com、Klook、Agoda 同一酒店可差 15-30%，必須比較。</li>
</ul>
</div>
...
<p><strong>Q: 邊個訂房平台最抵？</strong><br>A: 同一間酒店不同日子最抵平台都唔同，建議同時開三個 tab 格價先訂。</p>
<p><strong>Q: 需要訂金嗎？</strong><br>A: 預訂時會扣卡費（Non-refundable）或保留卡（Free Cancellation）。Check-in 時會再押金。</p>
</div>
...
for slug, d in enriched.items():
    ...
    path.write_text(render(slug, d), encoding="utf-8")
    count += 1

print(f"Enriched {count} directory pages with Wikipedia content + images")
